...
import {
  filterCryptoExchangeCredentials,
  isCryptoExchangeCredential,
  isQuickTradeExchangeCredential
...
test('keeps MT5 aliases available to MT5 flows without classifying them as crypto', () => {
  const credentials = [
    { id: 1, exchange_id: 'binance' },
    { id: 2, exchange_id: 'mt5' },
    { id: 3, exchange_id: 'cptmarkets' },
    { id: 4, exchange_id: 'cpt_markets' }
  ]

  assert.deepEqual(
    filterCryptoExchangeCredentials(credentials, 'mt5').map(item => item.id),
    [2, 3, 4]
  )
  assert.equal(isCryptoExchangeCredential(credentials[2]), false)
  assert.equal(isQuickTradeExchangeCredential(credentials[2]), true)
})
