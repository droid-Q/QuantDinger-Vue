...
const MT5_EXCHANGES = new Set(['mt5', 'cptmarkets', 'cpt_markets'])
...
export default {
  ...
  computed: {
    ...
    marketCategory () {
      ...
      const market = markets.length === 1 ? String(markets[0]) : 'Mixed'
      return market === 'MT5' ? 'Forex' : market
    },
    isPortfolioStrategy () {
    ...
    compatibleCredentials () {
      return this.credentials.filter(credential => {
        ...
        if (this.marketCategory === 'Forex') return MT5_EXCHANGES.has(exchange)
        return false
      })
    },
